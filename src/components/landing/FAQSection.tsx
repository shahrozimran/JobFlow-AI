...
export default function FAQSection() {
  ...
  return (
    <section className="py-32 bg-secondary/10 relative z-10 border-t border-border/40">
          ...
          <Accordion type="single" collapsible className="w-full">
            {faqs.map((faq, i) => (
              <AccordionItem key={i} value={`item-${i}`} className="border-border/40 px-2">
                <AccordionTrigger className="text-left text-lg font-medium py-6 hover:no-underline hover:text-muted-foreground transition-colors data-[state=open]:text-foreground">
                  {faq.q}
                </AccordionTrigger>
